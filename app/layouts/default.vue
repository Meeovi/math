<template>
  <v-app :theme="theme">
    ...
    <v-main class="bg-background">
      <div class="calculator-focus-container" tabindex="0" ref="calculatorContainer" @click="focusContainer"
        @keydown="handleKeydown" @focus="onContainerFocus" @blur="onContainerBlur">
        <v-container fluid class="fill-height pa-0 pa-sm-2">
          <!-- Standard Calculator (default) -->
          <StandardCalculator v-if="currentSection === 'standard'" class="full-height-component" ref="activeCalculator"
            @focus-request="focusContainer" />

          <!-- Scientific Calculator -->
          <ScientificCalculator v-else-if="currentSection === 'scientific'" class="full-height-component"
            ref="activeCalculator" @focus-request="focusContainer" />

          <!-- Programmer Calculator -->
          <ProgrammerCalculator v-else-if="currentSection === 'programmer'" class="full-height-component"
            ref="activeCalculator" @focus-request="focusContainer" />

          <!-- Date Calculation -->
          <DateCalculation v-else-if="currentSection === 'dateCalculation'" class="full-height-component"
            ref="activeCalculator" @focus-request="focusContainer" />

          <!-- Graphing Calculator -->
          <Graphing v-else-if="currentSection === 'graphing'" class="full-height-component" ref="activeCalculator"
            @focus-request="focusContainer" />

          <!-- Converters -->
          <CurrencyConverter v-else-if="currentSection === 'currency'" class="full-height-component"
            ref="activeCalculator" @focus-request="focusContainer" />
          <VolumeConverter v-else-if="currentSection === 'volume'" class="full-height-component" ref="activeCalculator"
            @focus-request="focusContainer" />
          <LengthConverter v-else-if="currentSection === 'length'" class="full-height-component" ref="activeCalculator"
            @focus-request="focusContainer" />
          <WeightMassConverter v-else-if="currentSection === 'weightMass'" class="full-height-component"
            ref="activeCalculator" @focus-request="focusContainer" />
          <TemperatureConverter v-else-if="currentSection === 'temperature'" class="full-height-component"
            ref="activeCalculator" @focus-request="focusContainer" />
          <EnergyConverter v-else-if="currentSection === 'energy'" class="full-height-component" ref="activeCalculator"
            @focus-request="focusContainer" />
          <AreaConverter v-else-if="currentSection === 'area'" class="full-height-component" ref="activeCalculator"
            @focus-request="focusContainer" />
          <SpeedConverter v-else-if="currentSection === 'speed'" class="full-height-component" ref="activeCalculator"
            @focus-request="focusContainer" />
          <TimeConverter v-else-if="currentSection === 'time'" class="full-height-component" ref="activeCalculator"
            @focus-request="focusContainer" />
          <PowerConverter v-else-if="currentSection === 'power'" class="full-height-component" ref="activeCalculator"
            @focus-request="focusContainer" />
          <DataConverter v-else-if="currentSection === 'data'" class="full-height-component" ref="activeCalculator"
            @focus-request="focusContainer" />
          <PressureConverter v-else-if="currentSection === 'pressure'" class="full-height-component"
            ref="activeCalculator" @focus-request="focusContainer" />
          <AngleConverter v-else-if="currentSection === 'angle'" class="full-height-component" ref="activeCalculator"
            @focus-request="focusContainer" />
        </v-container>
      </div>
    </v-main>
...
<script setup>
  ...
  import ProgrammerCalculator from '../components/sections/calculator/programmer.vue'
  import DateCalculation from '../components/sections/calculator/dateCalculation.vue'
  import Graphing from '../components/sections/calculator/graphing.vue'
  import CurrencyConverter from '../components/sections/converter/currency.vue'
  import VolumeConverter from '../components/sections/converter/volume.vue'
  import LengthConverter from '../components/sections/converter/length.vue'
  import WeightMassConverter from '../components/sections/converter/weightMass.vue'
  import TemperatureConverter from '../components/sections/converter/temperature.vue'
  import EnergyConverter from '../components/sections/converter/energy.vue'
  import AreaConverter from '../components/sections/converter/area.vue'
  import SpeedConverter from '../components/sections/converter/speed.vue'
  import TimeConverter from '../components/sections/converter/time.vue'
  import PowerConverter from '../components/sections/converter/power.vue'
  import DataConverter from '../components/sections/converter/data.vue'
  import PressureConverter from '../components/sections/converter/pressure.vue'
  import AngleConverter from '../components/sections/converter/angle.vue'
